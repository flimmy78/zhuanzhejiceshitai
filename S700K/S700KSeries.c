...
//    <0          失败
//------------------------------------------------------------------------------------------------
DWORD WINAPI S700K_MeasureRevision(LPT_Device pDevice, LPT_Record pData)
{
	double   dfDistance;
	DWORD    dwTicksError;
	DWORD    dwRetCode = 0;
...
//    <0          失败
//------------------------------------------------------------------------------------------------
int WINAPI S700K_ThreadRoutine( LPT_Device pDevice, LPT_Record pData )
{
	// 
	INFO1("\n\n================ 0.开始测试 ================");
	// 计时开始
...
//   DAQ_ACSelect (1);
	...
	// 转辙机动作过程检查，如果不成功退出
	INFO1("================ 1.动作检查 ================");
	if (S700K_DeviceCheck (pDevice, pData) == -1)
		goto Exit;
	...
	if (g_Device.Config.usLoad2Test == 1)
	{
		INFO1("================ 2.负载2测试 ================");		
		// 测试Load2，如果不成功退出
		if (S700K_MeasureLoad2 (pDevice, pData) == -1)
			goto Exit;
		// 如果要求停止测试，返回
		if (g_S700KData.Control.usTerminate)
			goto Exit;
	...
	if (g_Device.Config.usLoad1Test == 1)
	{
		// 测试Load1，如果不成功退出
		INFO1("================ 3.负载1测试 ================");			
		if (S700K_MeasureLoad1 (pDevice, pData) == -1)
			goto Exit;
		// 如果要求停止测试，返回
		if (g_S700KData.Control.usTerminate)
			goto Exit;
	...
	if (g_Device.Config.usLoad3Test == 1)
	{
		INFO1("================ 4.负载3测试 ================");			
		// 测试Load3，如果不成功退出
		if (S700K_MeasureLoad3 (pDevice, pData) == -1)
			goto Exit;
		// 如果要求停止测试，返回
		if (g_S700KData.Control.usTerminate)
			goto Exit;
	...
	{
		// 测试保持力，如果不成功退出
		if (S700K_MeasureRevision (pDevice, pData) == -1)
			goto Exit;
		// 如果要求停止测试，返回
		if (g_S700KData.Control.usTerminate)
			goto Exit;
	...
	{
		// 测试挤岔力，如果不成功退出
		if (S700K_MeasureTrailling (pDevice, pData) == -1)
			goto Exit;
		// 如果要求停止测试，返回
		if (g_S700KData.Control.usTerminate)
			goto Exit;
